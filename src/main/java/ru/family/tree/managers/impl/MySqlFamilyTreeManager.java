 ...
 */
public class MySqlFamilyTreeManager implements FamilyTreeManager {
    ...
    @Override
    public FamilyTree createFamilyTree(final long parentId, final long childId) {
        return transactionOperations.execute(status -> {
            final long familyId = createFamily();
            addChildrenInternal(familyId, parentId, Collections.singleton(childId));
            return loadInternal(familyId);
        });
    }

    @Override
    public FamilyTree createFamilyTree(final FamilyTree familyTree) {
        return transactionOperations.execute(status -> {
            final long familyId = createFamily();
            final Multimap<Person, Person> parentsToChildren = familyTree.  getParentsToChildren();
    ...
    @Override
    public FamilyTree load(final long familyId) {
        return loadInternal(familyId);
    }

    @Override
    public FamilyTree loadByPerson(final long personId) {
    ...
    private long createFamily() {
        ...
        jdbcOperations.update(
                factory.newPreparedStatementCreator(EMPTY_PARAMS),
                keyHolder
        );
    ...
    private FamilyTree loadInternal(final long familyId) {
        ...
        for (final SimpleFamilyRelation relation : sortedRelations) {
            builder.addRelation(personById.get(relation.parentId), personById.get(relation.childId));
        }
